...
class Dashboard(object):
    ...
    def start(self):
        """Start the dashboard."""
        self._viz = Visdom(
            server=self.visdom_addr, port=self.visdom_port, env=self.env_name
        )
...
def start_visdom_server() -> subprocess.Popen:
    ...
    visdom_server_args = [
        ...
        "visdom.server",
        "-env_path",
        os.path.join(CUR_DIR, "..", ".visdom_env"),
    ]
    print(" ".join(visdom_server_args))
    prog = subprocess.Popen(
        visdom_server_args, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
    )
    time.sleep(1.0)
