...
PROPOSAL_ID = Tuple[DIALOGUE_LABEL, int]  # ((origin, dialogue_id), message_id)
ACCEPTANCE_ID = Tuple[DIALOGUE_LABEL, int]  # ((origin, dialogue_id), message_id)
...
class BaselineAgent(NegotiationAgent):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._pending_proposals = dict()  # type: Dict[PROPOSAL_ID, Description]
        self._pending_acceptances = dict()  # type: Dict[ACCEPTANCE_ID, Description]
    ...
    def _on_cfp_as_seller(self, msg_id: int, dialogue_id: int, origin: str, target: int, query: CFP_TYPES) -> None:
        ...
        if not query.check(goods_supplied_description):
            ...
            self._pending_proposals[proposal_id] = goods_supplied_description
    ...
    def _on_propose_as_buyer(self, msg_id: int, dialogue_id: int, origin: str, target: int, proposals: PROPOSE_TYPES) -> None:
        ...
        if proposal_delta_score > self._fee:
            logger.debug("[{}]: Accepting propose with proposal_delta_score={}, price={}, fee={}"
                         .format(self.public_key, proposal_delta_score, price, self._fee))
    ...
    def on_accept(self, msg_id: int, dialogue_id: int, origin: str, target: int) -> None:
        ...
        acceptance_id = (dialogue_label, target)
        if acceptance_id in self._pending_acceptances:
    ...
    def on_match_accept(self, msg_id: int, dialogue_id: int, origin: str, target: int):
        ...
        dialogue_label = (origin, dialogue_id)  # type: DIALOGUE_LABEL
        acceptance_id = (dialogue_label, target)
        assert acceptance_id in self._pending_acceptances
        proposal = self._pending_acceptances.pop(acceptance_id)
        price, quantity_by_good_id = self._extract_info_from_propose(proposal)
    ...
    def on_transaction_confirmed(self, tx_confirmation: TransactionConfirmation) -> None:
        super().on_transaction_confirmed(tx_confirmation)
